...
def show_calculator():
    # Ввод уровня
    frame_level = tk.Frame(root, bg='#2e2e2e')
    frame_level.pack(fill=tk.X)
    
    label_level = tk.Label(frame_level, text="Введите уровень героя (1-30):  ", fg='ivory', bg='#2e2e2e')
    entry_level = tk.Entry(frame_level, bg='#1f1f1f', fg='ivory', insertbackground='ivory')
    
    label_level.pack(side=tk.LEFT, anchor=tk.W)
    entry_level.pack(side=tk.LEFT, padx=5, anchor=tk.W)
    entry_level.insert(0, "1")

    # Ввод опыта
    frame_exp = tk.Frame(root, bg='#2e2e2e')
    frame_exp.pack(fill=tk.X)
    
    label_exp = tk.Label(frame_exp, text="Введите оставшийся опыт:       ", fg='ivory', bg='#2e2e2e')
    entry_exp = tk.Entry(frame_exp, bg='#1f1f1f', fg='ivory', insertbackground='ivory')
    
    label_exp.pack(side=tk.LEFT, anchor=tk.W)
    entry_exp.pack(side=tk.LEFT, padx=5, anchor=tk.W)
    entry_exp.insert(0, "0")

    # Ввод серии убийств
    frame_streak = tk.Frame(root, bg='#2e2e2e')
    frame_streak.pack(fill=tk.X)
    
    label_streak_input = tk.Label(frame_streak, text="Введите серию убийств:            ", fg='ivory', bg='#2e2e2e')
    entry_streak = tk.Entry(frame_streak, bg='#1f1f1f', fg='ivory', insertbackground='ivory')
    
    label_streak_input.pack(side=tk.LEFT, anchor=tk.W)
    entry_streak.pack(side=tk.LEFT, padx=5, anchor=tk.W)
    ...
    def calculate():
        try:
            ...
            # Обновляем метки и показываем их
            label_result.config(text=f"Общий опыт: {total_exp}")
            
            label_allpick.config(text="\n".join(allpick_rewards))
            label_turbo.config(text="\n".join(turbo_rewards))
            
            # Показываем метки (если были скрыты)
            label_result.pack(pady=(10,2), fill=tk.X)
            label_allpick_title.pack(pady=(10,2), fill=tk.X)
            label_allpick.pack(pady=(2,10), fill=tk.X)
            label_turbo_title.pack(pady=(10,2), fill=tk.X)
            label_turbo.pack(pady=(2,10), fill=tk.X)
...
# Вызов функции отображения калькулятора без стартовой кнопки
show_calculator()
